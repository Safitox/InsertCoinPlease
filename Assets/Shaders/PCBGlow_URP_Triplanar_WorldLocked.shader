Shader "Custom/PCBGlow_URP_Triplanar_WorldLocked"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 SampleTriplanar(TEXTURE2D_PARAM(tex, samp), float3 worldPos, float3 worldNormal, float tileSize, float blendSharpness)
            {
                ...
                float2 uvX = worldPos.zy / tileSize; // project onto YZ when facing +X/-X
                float2 uvY = worldPos.xz / tileSize; // onto XZ when facing +Y/-Y
                float2 uvZ = worldPos.xy / tileSize; // onto XY when facing +Z/-Z
            ...
            float StripeMask(float3 worldPos, float period, float width, float angleYDeg, float time, float speed)
            {
                ...
                // Build a soft box around 0.5 using smoothstep to avoid aliasing
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // Simple lambert for some shape (optional; keep unlit look if desired)
